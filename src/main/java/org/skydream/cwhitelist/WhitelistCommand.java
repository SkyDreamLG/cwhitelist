...
public class WhitelistCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        ...
        dispatcher.register(Commands.literal("cwhitelist")
                .executes(ctx -> {
                    ctx.getSource().sendSuccess(() -> Cwhitelist.translate(ctx.getSource().getPlayer(),"cwhitelist.help"), false);
                    return 1;
                })
                                ...
                                .then(Commands.argument("value", StringArgumentType.word())
                                        .executes(ctx -> {
                                            ...
                                            if (!hasPermission(ctx.getSource(), PERMISSION_ADD)) {
                                                ctx.getSource().sendFailure(Cwhitelist.translate(ctx.getSource().getPlayer(),
    ...
    private static boolean checkPermission(CommandSourceStack source, PermissionNode<Boolean> permission, String action) {
        ...
        source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.error.no_permission", action));
        return false;
    }
    ...
    private static int addEntry(CommandSourceStack source, String type, String value) {
        ...
        if (!Arrays.asList("name", "uuid", "ip").contains(type.toLowerCase())) {
            source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.error.invalid_type", type));
            return 0;
        }
        ...
        if (WhitelistManager.containsEntry(entry)) {
            source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.error.entry_exists", type, value));
            return 2;
        }
        try {
            WhitelistManager.addEntry(entry);
            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.success.entry_added", type, value), true);
            if (WhitelistManager.isUsingApi()) {
                source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.info.added_to_api"), false);
            } else {
                source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.info.added_to_local"), false);
            }
            return 1;
        } catch (Exception e) {
            source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.error.add_failed", e.getMessage()));
            return 3;
        }
    }
    ...
    private static int removeEntry(CommandSourceStack source, String type, String value) {
        ...
        if (removed) {
            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.success.entry_removed", type, value), true);
            if (WhitelistManager.isUsingApi()) {
                source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.info.removed_from_api"), false);
            } else {
                source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.info.removed_from_local"), false);
            }
        } else {
            source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.error.entry_not_found", type, value));
        }
        return removed ? 1 : 0;
    }
    ...
    private static int listEntries(CommandSourceStack source) {
        ...
        if (Config.ENABLE_API.get()) {
            if (WhitelistManager.isUsingApi()) {
                ...
                if (tokenInfo != null) {
                    message = Cwhitelist.translate(source.getPlayer(),"cwhitelist.list.api_with_token",
                            tokenInfo.name,
                            tokenInfo.canRead ? "✓" : "✗",
                            tokenInfo.canWrite ? "✓" : "✗",
                            tokenInfo.canDelete ? "✓" : "✗",
                    ...
                    message = Cwhitelist.translate(source.getPlayer(),"cwhitelist.list.api_mode",
                            WhitelistManager.getEntryCount());
                }
            } else {
                message = Cwhitelist.translate(source.getPlayer(),"cwhitelist.list.api_unavailable",
                        WhitelistManager.getEntryCount());
            }
        } else {
            message = Cwhitelist.translate(source.getPlayer(),"cwhitelist.list.local_mode",
                    WhitelistManager.getEntryCount());
        }
        if (WhitelistManager.getEntryCount() > 0) {
            MutableComponent entriesList = Component.literal("\n");
            WhitelistManager.getEntries().forEach(e ->
                    entriesList.append(Cwhitelist.translate(source.getPlayer(),"cwhitelist.list.entry_item",
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                source.getServer().execute(() -> {
                    source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.reload.success"), false);
                    if (Config.ENABLE_API.get()) {
                        if (WhitelistManager.isUsingApi()) {
                            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.reload.source_api"), false);
                        } else {
                            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.reload.source_local"), false);
                        }
                    }
                });
    ...
    private static int apiStatus(CommandSourceStack source) {
        ...
        Component status = Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.status.title",
    ...
    private static int apiHealthCheck(CommandSourceStack source) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                ...
                source.getServer().execute(() -> {
                    if (healthy) {
                        source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.health.success"), false);
                    } else {
                        source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.health.failed"));
                    }
                });
            } catch (Exception e) {
                source.getServer().execute(() -> {
                    source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.health.error",
            ...
            try {
                ...
                source.getServer().execute(() -> {
                    if (verified) {
                        ApiClient.TokenInfo tokenInfo = ApiClient.getTokenInfo();
                        if (tokenInfo != null) {
                            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.verify.success"), false);
                            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.verify.token_info",
                                    tokenInfo.name,
                                    tokenInfo.canRead ? "✓" : "✗",
                                    tokenInfo.canWrite ? "✓" : "✗",
                        ...
                        source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.verify.failed"));
                    }
                });
            } catch (Exception e) {
                source.getServer().execute(() -> {
                    source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.verify.error",
                            e.getCause() != null ? e.getCause().getMessage() : e.getMessage()));
                });
            }
        });
            ...
            try {
                ...
                source.getServer().execute(() -> {
                    if (entries != null && !entries.isEmpty()) {
                        WhitelistManager.reload();
                        source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.sync.success", entries.size()), false);
                        if (WhitelistManager.isUsingApi()) {
                            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.reload.source_api"), false);
                        } else {
                            source.sendSuccess(() -> Cwhitelist.translate(source.getPlayer(),"cwhitelist.reload.source_local"), false);
                        ...
                        source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.sync.no_entries"));
                    }
                });
            } catch (java.util.concurrent.TimeoutException e) {
                source.getServer().execute(() -> {
                    source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.sync.timeout"));
                });
            } catch (Exception e) {
                source.getServer().execute(() -> {
                    source.sendFailure(Cwhitelist.translate(source.getPlayer(),"cwhitelist.api.sync.error",
                            e.getCause() != null ? e.getCause().getMessage() : e.getMessage()));
                });
            }
        });
